        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_400px] items-start pb-16">
          ...
          <aside className="lg:sticky lg:top-6 space-y-4 rise" style={{ animationDelay: "320ms" }}>
            <div className="slide-preview">
              ...
              {(hovered ?? selected) ? (
                <div key={`pic-${(hovered ?? selected)!.id}`} className="mx-auto max-w-[220px] mb-5"><Diagram id={(hovered ?? selected)!.diagram} marks={(hovered ?? selected)!.marks} /></div>
              ) : !noteBusy && regionView !== "body" ? (
                <div key={`pic-region-${regionView}`} className="mx-auto max-w-[220px] mb-5" aria-hidden><Diagram id={regionView} /></div>
              ) : (
              ...
              {(hovered?.plain || customHeadline || selected?.plain) && (
                <p key={`text-${hovered?.id ?? "chosen"}`} className="display-sm" style={{ fontSize: "1.6rem" }}>{hovered?.plain || customHeadline || selected?.plain}</p>
              )}
